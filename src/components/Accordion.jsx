...
const Accordion = ({
  ...
  const removePerson = (index) => {
    ...
    setPeople(newPeople);
    window.localStorage.setItem('list', JSON.stringify(newPeople));
  };
  ...
  function getBackgroundColor(index) {
    switch (index) {
      case 0:
        return 'bg-green-600';
      case 1:
        return 'bg-cyan-700'; // Change this to the desired color for index 1
      case 2:
        return 'bg-sky-900';
      case 3:
        return 'bg-blue-900';
    ...
    <div className='accordion w-[782px]  bg-white text-[#3A719B] mb-1'>
      <div className='accordion-item w-'>
        ...
        <div className='accordion-title flex h-[64px]'>
          {/* index number */}
          <div
            className={`w-10 h-16 ${getBackgroundColor(
              index
            )} flex items-center justify-center`}
          >
            <span className='w-[15px] h-[16px] text-white'>{index + 1}</span>
          </div>
          ...
          <div className='flex flex-grow'>
            ...
            <div className='flex  flex-grow h-16 w-20 items-center justify-end pr-6'>
              <div>
                <span onClick={() => removePerson(index)}>
                  <svg
